...
export async function GET(request: Request) {
  ...
  if (!targetCategory) {
    return NextResponse.json({ 
      hata: "Lütfen URL sonuna kategori parametresi ekleyin. Örnek: ?kategori=koltuk, ?kategori=kose, ?kategori=berjer, ?kategori=puf" 
    }, { status: 400 });
  }
  ...
  const desi = '250';
  const kargoSuresi = '2';
  // kargoTipi değişkenini kaldırdık çünkü Trendyol Excel'inde bu alan boş kalmalı.
  ...
  if (targetCategory === 'koltuk') {
    filteredProducts = products.filter(p => ['koltuk-takimlari', 'modern-koltuk-takimlari', 'chester-koltuk-takimlari'].includes(p.categorySlug));
    categoryId = '2422';
  ...
  filteredProducts.forEach((product) => {
    ...
    const row = activeHeaders.map((header) => {
      switch (header) {
        ...
        case 'Sevkiyat Tipi': return ''; // DÜZELTİLDİ: Trendyol Excel'inde boş bırakılır
        
        case 'Üretici Adı': return marka; // YENİ EKLENDİ: Veluxe Home & Living otomatik dolar
        ...
        case 'Materyal': 
          if (targetCategory === 'koltuk' || targetCategory === 'berjer') return 'Hakiki Deri';
